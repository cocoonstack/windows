...
- The `viosock.inf` driver package is in the driver store (`pnputil /enum-drivers`). Required for `cocoon-agent`'s `AF_VSOCK` listener; `virtio-win-guest-tools.exe /S` does not include it, so a separate `pnputil /add-driver viosock.inf /install` runs at firstboot. Device bind + Winsock catalog (`Virtio Vsock STREAM` AF=40) are not checked at build time because the build-time QEMU exposes no vsock device — driver autobinds when production CH does.
- The `cocoon-agent` Windows service is registered with `start=auto` (`Get-Service cocoon-agent`) and starts at first boot under production CH. The build-time QEMU has no vsock device, so the service registers but does not run there — the verifier checks registration, not running state. Pinned to a specific `cocoon-agent` release (currently v0.1.2); bumping requires updating `$version` + `$expected` SHA256 in `scripts/install-cocoon-agent-bootstrap.ps1`.
...
| 53     | **cocoon-agent install**     | `install-cocoon-agent-bootstrap.ps1` (lives on the install ISO root, copied there by `build-qemu.sh` at repack time): download pinned `cocoon-agent_v0.1.5_Windows_x86_64.zip` from GitHub Releases, verify SHA256, run bundled `install-cocoon-agent.ps1` (registers `cocoon-agent` Windows service: LocalSystem, auto-start, restart-on-crash, vsock port 1024). Self-copies to `C:\Scripts\install-cocoon-agent-bootstrap.ps1` for `remediate.ps1` re-invoke. |
| 54     | **NIC auto-heal task**       | `setup-cocoon-nic-autoheal.ps1` (on the install ISO root): copy `cocoon-nic-autoheal.ps1` from the ISO to `C:\CocoonNicAutoHeal.ps1` and register the `CocoonNicAutoHeal` scheduled task (1-minute, SYSTEM, HIGHEST). Cycles all Net PnP devices to recover chained-clone NDIS state. |
